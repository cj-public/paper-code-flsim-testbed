...
class Worker:
    ...
    def __init__(self, worker_name, message, run_type, sibling_worker_num):
        ...
        task_info = query_database(TABLE_FEDASYNC_TASK_POOL, params)[0]
    ...
    def prepare_base_weights(self):
        print_log('prepare base weights')
        ...
        # 不做优化
        # my_weights_info_list = []

        # 如果每个worker刚好前序weights未被aggregate，则worker不会被激活，而master不会因为task publish被启动，所以会造成死锁
        ...
        latest_global_weights_info = query_database(TABLE_FEDASYNC_WEIGHTS_POOL, params)[0]

        params = {'conditions': {'id': latest_global_weights_info['id']}, 'order': {'key': 'create_time', 'value': 'desc'}, 'limit': 1}
        wtinfo = query_database(TABLE_FEDASYNC_WEIGHTS_POOL, params)[0]
...
def process_message(worker_names, message, run_type):
    ...
    for selected_worker_name in virtual_worker_names[:1]:
        ...
        if message['event'] == EVENT_GLOBAL_WEIGHTS_UPLOADED:

            # 已一定概率忽略，否则资源占用率过高
            # NOTE 其实只有一个master发出global_weights_upload_event，所以还好
