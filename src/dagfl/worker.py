...
class Worker:
    ...
    def __init__(self, worker_name, message, run_type, sibling_worker_num):
        ...
        fastdfs_cli.set_runner_info(FL_SYSTEM_DAGFL, self.name)
        ...
        task_info_list = query_database(TABLE_DAGFL_TASK_POOL, params)
        ...
        params = {'conditions': {'task_id': self.task_id, 'fl_system': FL_SYSTEM_DAGFL, 'runner_name': self.name}, 'limit': 1}
    ...
    def prepare_base_weights(self):
        ...
        candidate_weights_info_list = query_database(TABLE_DAGFL_WEIGHTS_POOL, params)
        ...
        # NOTE 发现一个有趣的现象，不能排除自己训练的weights，否则训练效果极差；
        # 之前代码有bug（loss采用desc排序），不存在上述现象
        ...
        # weights有时效性，只保留最新有效的weights
        staleness_threshold = self.framework_params['staleness_threshold']
        fresh_weights_info_list = [cw for cw in candidate_weights_info_list if current_timestamp() - datetime_to_timestamp(cw['create_time']) < staleness_threshold]

        # 有些时候（如初始时、网络抖动比较厉害时）所有的weights都不满足，需要忽略时效性的限制
        if len(fresh_weights_info_list) == 0:
            candidate_weights_info_list = candidate_weights_info_list[-1:]
        else:
            candidate_size = self.framework_params['candidate_size']
            candidate_weights_info_list = candidate_weights_info_list[-candidate_size:]
        ...
        # update_weights_aggregated(TABLE_DAGFL_WEIGHTS_LOCAL_EVALUATION, selected_weights_ids)
        for wi in selected_weights_ids:
            update_weights_local_evaluation_aggregated(wi, self.dataset_id)
        
        update_weights_aggregated(TABLE_DAGFL_WEIGHTS_POOL, selected_weights_ids)
        ...
        contribute_weights(FL_SYSTEM_DAGFL, weights_config, True)
    ...
    def run(self):
        ...
        exec_hist['fl_system'] = FL_SYSTEM_DAGFL
        ...
        # 严格来说，is_aggregated得设置成0，否则在最终统计时比较吃亏，因为有一部分weights uploaded之后，并没有被aggregate，单也算成有效的iteration
